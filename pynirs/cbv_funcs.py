...
def eckartraw(input1, input2, doplot=False):
    fft1 = sp.fftpack.fft(input1)
    fft2 = sp.fftpack.fft(input2)
    ...
    g = np.real(sp.fftpack.fftshift(sp.fftpack.ifft(G)))
    # g=np.real(sp.fftpack.ifft(G))
    if doplot:
...
def risetime_eval(x, p):
    corrx = x - p[0]
    return 0.5 * (1.0 + np.sign(corrx)) * p[1] * (1.0 - np.exp(-corrx / p[2]))
...
@conditionaljit()
def findrisetimefunc(thexvals, theyvals, debug=False, refine=False, displayplots=False):
    # guess at parameters: risestart, riseamplitude, risetime

    # guess that the last points in y are at the max value
    initriseamplitude = np.mean(theyvals[-5:-1])

    # find the 25% and 75% points
    ind25 = findfirstabove(theyvals, 0.25 * initriseamplitude)
    ind75 = findfirstabove(theyvals[ind25:], 0.75 * initriseamplitude) + ind25
    initrisetime = thexvals[ind75] - thexvals[ind25]
...
def doprecalcfftfilt(filterfunc, indata):
    indata_trans = sp.fftpack.fft(indata)
    indata_trans = indata_trans * filterfunc
    return sp.fftpack.ifft(indata_trans).real
